...
portfolio(where: {status: {status: {_neq: "Available"}}, type: {_eq: "Lift"}}) {
  ...
  res = requests.post("https://nr-lift-and-escalator.azure-api.net/gateway/v1/", json={"query": ALL_STATIONS})
  res.raise_for_status()
  print (len(res.json()["data"]["portfolio"]))
  for lift in res.json()["data"]["portfolio"]:
     yield {"status": lift["status"]["status"], "location": lift["location"], "station": lift["station"]}
...
def tflapi_lift_issues():
  ...
  for issue in res.json():
    if issue["description"].lower().find("lift") != -1:
      yield hashdict({"status": issue["description"].strip(), "location": None, "station": issue["commonName"].replace(" Station", "")})

def tflapi_lift_disruptions():
  res = requests.get("https://api.tfl.gov.uk/Disruptions/Lifts/")
  res.raise_for_status()
  for issue in res.json():
    yield hashdict({"status": issue["message"].strip(), "location": None, "station": issue["stopPointName"].replace(" Station", "")})
